...
public class Yoda20220310GetFunctionSummury extends GhidraScript {
        ...
        while (iter.hasNext() && !monitor.isCancelled()) {
            ...
            if (sym != null) {//
        		...
        		for(int i=0; i<refs.length;i++) {             			
        			...
        			addCondidateCount();
        			decompileFunction2(refFunc, decompApi);
        		}
    ...
    public DecompileResults decompileFunction2(Function f, FlatDecompilerAPI flatApi) {
        ...
        try {
            ...
            removeWords.add("DAT");
            removeWords.add("stack");
            removeWords.add("param");
            removeWords.add("pcVar");
            removeWords.add("char");
            removeWords.add("while");
            removeWords.add(" ");
            String functionSummury = "";
            for(String str: decompiled) {
            	//mac: デコンパイル結果に埋め込み文字列がでてくるが，winはPTR__で表示されるので注意
            	boolean replaced = false;
            	for(String rm: removeWords) {
            		if(str.contains(rm)) {
            			functionSummury += str.replace(rm, "");
            			replaced = true;
            		}
            	}
            	if(!replaced) {
            ...
            //println(f.getName());
            //println(functionSummury+"\n\n");
			logger.info(f.getName());
			logger.info(functionSummury+"\n\n");
