...
public class Yoda20220208_ScanfPrintf extends GhidraScript {
    ...
    public void run() throws Exception {
        ...
        String homepath = "C:/Users/MinamiYoda/Documents/Program/firmware/docker/result/files/Yoda20220208_ScanfPrintf/"+date.format(formatter_day)+"/";
        ...
        DateTimeFormatter formatter_day = DateTimeFormatter.ofPattern("yyyy-MM-dd");
        DateTimeFormatter formatter_time = DateTimeFormatter.ofPattern("HH-mm");
        String logDirPath = "C:/Users/MinamiYoda/Documents/Program/firmware/docker/result/files/Yoda20220208_ScanfPrintf/";
        File logDirectory = new File(logDirPath);
        if (!logDirectory.exists()){
            boolean dir_made = logDirectory.mkdirs();
            if(dir_made) {
                println(logDirectory.toString());
            }
        }
        String homepath = "C:/Users/MinamiYoda/Documents/Program/firmware/docker/result/files/Yoda20220208_CmdStrSearch/"+date.format(formatter_day)+"/";
        String projectName = this.getProgramFile().getName();
        String FirmwareMaker = this.getProgramFile().getPath();
        FirmwareMaker = FirmwareMaker.substring(61).replace("\\", "_").replace(".bin", "");
        File directory = new File(homepath + FirmwareMaker);
        ...
        searchedCount = 0;
        candidateCount = 0;
        String logFilePath =  directory.toString() + "/" + date.format(formatter_time) + "-stringsearch.log";
		//logger
        try {  
        ...
        */
        ...
        while (iter.hasNext() && !monitor.isCancelled()) {
            ...
            if (sym != null && (sym.getName().matches("printf") || sym.getName().matches("scanf"))) {
